...
DEMO_DBG_LEVEL_e demo_debug_level = DEMO_DBG_ERR;
...
{
    {
        ...
        .name      = "extention cus ctrol",
        .entity    = UVC_AIT_EU1_GUID,
        .selector  = UVC_CS_EXTENSION_CUS_CONTROL,
        .size      = UVC_CS_EXTENSION_LEN,
        .offset    = 0,
...
{
    {
        .id        = V4L2_CID_PAN_RELATIVE,
        .name      = "RESET_I_FREAME",
        .entity    = UVC_CUS_EU2_GUID,
        .selector  = CUS_XU_SET_ISP,
        .size      = CUS_XU_ISP_LEN,
        .offset    = 0,
        .v4l2_type = V4L2_CTRL_TYPE_INTEGER,
        .data_type = UVC_CTRL_DATA_TYPE_SIGNED,
...
int query_xu_control_init(DeviceContex_t *ctx)
{
    ...
    for ( i=0; i < sizeof(xu_mappings)/sizeof(struct uvc_xu_control_mapping); i++ )
    {
        DEMO_INFO("V4L2_CORE: mapping control for %s\n", xu_mappings[i].name);

        if ((err=ioctl(s->fd, UVCIOC_CTRL_MAP, &xu_mappings[i])) < 0)
        {
            if ((errno!=EEXIST) && (errno != EACCES))
            {
                DEMO_ERR("V4L2_CORE: (UVCIOC_CTRL_MAP) Error: %s %d\n", strerror(errno), EEXIST);
            }
            else if (errno == EACCES)
            {
                DEMO_ERR("V4L2_CORE: need admin previledges for adding extension controls\n");
                DEMO_ERR("V4L2_CORE: please run 'guvcview --add_ctrls' as root (or with sudo)\n");
                return  (-1);
            }
            else
                DEMO_INFO("V4L2_CORE: Mapping exists: %s\n", strerror(errno));
        }
    }
    return err;
}

uint16_t v4l2core_get_length_xu_control(int fd, uint8_t unit, uint8_t selector)
{
    uint16_t length = 0;
    ...
    struct uvc_xu_control_query xu_ctrl_query =
    {
        .unit     = unit,
        .selector = selector,
        .query    = UVC_GET_LEN, //get_len size must be 2 byte
        .size     = sizeof(length),
        .data     =  (uint8_t *) &length
    };

    if (ioctl(fd, UVCIOC_CTRL_QUERY, &xu_ctrl_query) < 0)
    {
        DEMO_ERR("V4L2_CORE: UVCIOC_CTRL_QUERY (GET_LEN) - Error: %s\n", strerror(errno));
        return 0;
    }
...
int query_xucontrol_query(DeviceContex_t *ctx, uint8_t query, uint8_t unit, u_int8_t selector, void *data)
{
    ...
    if (query==UVC_GET_LEN)
        len = 2;
    else if (query==UVC_GET_INFO)
        len = 1;
    else
        len = v4l2core_get_length_xu_control(s->fd, unit, selector);
    ...
    printf("---- V4L2_CORE unit=%d selector=%d len=%d \n",unit,selector,len);
    /*get query data*/
    if ((err=ioctl(s->fd, UVCIOC_CTRL_QUERY, &xu_ctrl_query)) < 0)
    {
        DEMO_ERR("V4L2_CORE: UVCIOC_CTRL_QUERY (%i) - Error: %s\n", query, strerror(errno));
    }
...
static int mmap_init(DeviceContex_t *ctx)
{
    ...
    s->buffers = req.count;
    s->buf_start = calloc(1, s->buffers * sizeof(void *));
    if (!s->buf_start) {
        DEMO_ERR("Cannot allocate buffer pointers\n");
        return -ENOMEM;
    }
    s->buf_len = calloc(1, s->buffers * sizeof(unsigned int));
    if (!s->buf_len) {
...
static unsigned int image_get_buffer_size(int fmt, int width, int height, int align)
{
    ...
    case V4L2_PIX_FMT_H264:
        buffer_size=( 240 * 180 * 10.0+240);//width * height * 2.0 / 7; 
        break;
...
int v4l2_read_start_test(DeviceContex_t *ctx)
{
    struct video_data *s = ctx->priv_data;
    int res = 0;
...
int v4l2_read_close_test(DeviceContex_t *ctx)
{
    struct video_data *s = ctx->priv_data;
...
int v4l2_dev_init(DeviceContex_t **ctx, char *path)
{
    ...
    DeviceContex_t *c = 
       calloc(1, sizeof(DeviceContex_t));
    struct video_data *s = 
       calloc(1, sizeof(struct video_data));
